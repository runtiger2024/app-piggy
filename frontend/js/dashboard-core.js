...
window.loadUserProfile = async function () {
  try {
    ...
    // 只有在 API 回傳非 200 (如 401 Unauthorized) 時才執行登出動作
    if (!response.ok) {
    ...
    // --- 安全更新 DOM (修正點：加入存在性檢查) ---
    // 這樣在不含這些 ID 的頁面 (例如代採購頁) 就不會報錯導致登出
    ...
    if (phoneEl) {
      phoneEl.textContent = window.currentUser.phone || "(未填寫)";
    }

    const addressEl = document.getElementById("user-address");
    if (addressEl) {
      addressEl.textContent = window.currentUser.defaultAddress || "(未填寫)";
    }
  } catch (error) {
    console.error("User profile error:", error);
    // 只有當真的是授權問題或網路中斷時才導向登入，避免 DOM 操作錯誤觸發登出
